...
if ($_POST) {
	...
	$result = db_query("SELECT
		...
		WHERE (SELECT SUM(h.hrs) FROM _josh_hours h WHERE h.projectID = p.id AND $timeframe) IS NOT NULL
	...
	$employees = db_query("SELECT 
			...
			e.[first],
			e.[last],
			(SELECT SUM(h.hrs) FROM _josh_hours h WHERE h.employeeID = e.id AND $timeframe) total
		FROM employee e 
		WHERE
			e.[last] IS NOT NULL AND
			(SELECT SUM(h.hrs) FROM _josh_hours h WHERE h.employeeID = e.id AND $timeframe) IS NOT NULL
	...
	while ($e = db_fetch($employees)) {
		...
		$hours = db_query("SELECT
					...
					h.hrs
				FROM _josh_hours h
				WHERE h.employeeID = {$e["id"]} AND $timeframe");
